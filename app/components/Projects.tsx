    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto z-80">
        {projects.slice(0, 3).map((project) => (
          ...
          >
            <div
              className={`w-full h-full ${project.theme} flex items-center justify-center relative`}
            >
              <span
                className={`font-glitch text-9xl ${project.text} opacity-50 group-hover:scale-150 transition-transform duration-500`}
              >
                {project.number}
              </span>
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-void-black/80 backdrop-blur-sm">
                <span className="font-(--font-space-grotesk) text-dirty-white text-xl uppercase tracking-widest border-b-2 border-acid-green pb-1">
                  {t.view_project}
                </span>
              </div>
            </div>
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedProject.tech.map((tech) => (
                    <span
                      key={tech}
                      className="border border-dirty-white/30 px-3 py-1 text-sm font-mono hover:bg-dirty-white hover:text-void-black transition-colors cursor-crosshair"
                    >
                      {tech}
                    </span>
                  ))}
