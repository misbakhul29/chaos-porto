...
export default function SpaceshipCursor() {
  ...
  const [shootingEnabled, setShootingEnabled] = useState(false);
  ...
  useEffect(() => {
    bgmRef.current = new Audio(
      "/sounds/good_b_music-perfect-beauty-191271.mp3",
    );
    if (bgmRef.current) {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (Math.abs(dx) > 1 || Math.abs(dy) > 1)
        setShipRotation(Math.atan2(dy, dx) * (180 / Math.PI) + 90);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-9999 overflow-hidden">
      <div className="absolute top-20 right-8 flex flex-col items-end gap-3 font-glitch select-none mix-blend-difference">
        <div className="flex flex-col-reverse gap-6 items-center">
          {hudEnabled && (
            <div className="relative py-2 px-4 border-r-4 border-acid-green bg-acid-green/5 group">
              ...
              <div className="text-[10px] font-mono text-dirty-white/40 mb-1 tracking-[0.2em]">
                STATUS_CONNECTED
              </div>
          ...
          <div className="flex gap-2 items-center pointer-events-auto">
            <div className="relative group/tooltip">
              ...
              >
                ...
                <div className="relative z-10">
                  {isPlaying ? (
                    <Volume2
                      className="text-dirty-white group-hover:text-acid-green"
                      size={16}
                    />
                  ) : (
                    <VolumeX className="text-hot-pink" size={16} />
                  )}
                </div>
            ...
            <div className="relative group/tooltip">
              <button
                ...
                className={`group relative flex items-center justify-center w-10 h-10 border transition-all duration-300 overflow-hidden ${spawnEnabled ? "border-dirty-white/30 hover:border-hot-pink" : "border-hot-pink bg-hot-pink/15"}`}
              >
                <div className="absolute inset-0 bg-hot-pink/0 group-hover:bg-hot-pink/10 transition-colors"></div>
                <span
                  className={`font-mono text-[9px] font-bold ${spawnEnabled ? "text-dirty-white/70 group-hover:text-hot-pink" : "text-hot-pink animate-pulse"}`}
                >
                  METR
                </span>
                <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-dirty-white/20 -rotate-45 translate-x-1/2 -translate-y-1/2"></div>
            ...
            <div className="relative group/tooltip">
              <button
                onClick={() => setShootingEnabled((prev) => !prev)}
                className={`group relative flex items-center justify-center w-10 h-10 border transition-all duration-300 overflow-hidden ${shootingEnabled ? "border-dirty-white/30 hover:border-electric-blue" : "border-electric-blue bg-electric-blue/15"}`}
              >
                <div className="absolute inset-0 bg-electric-blue/0 group-hover:bg-electric-blue/10 transition-colors"></div>
                <span
                  className={`font-mono text-[9px] font-bold ${shootingEnabled ? "text-dirty-white/70 group-hover:text-electric-blue" : "text-electric-blue animate-pulse"}`}
                >
                  WEAP
                </span>
                <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-dirty-white/20 -rotate-45 translate-x-1/2 -translate-y-1/2"></div>
            ...
            <div className="relative group/tooltip">
              <button
                onClick={() => setHudEnabled((prev) => !prev)}
                className={`group relative flex items-center justify-center w-10 h-10 border transition-all duration-300 overflow-hidden ${hudEnabled ? "border-dirty-white/30 hover:border-acid-green" : "border-acid-green bg-acid-green/15"}`}
              >
                <div className="absolute inset-0 bg-acid-green/0 group-hover:bg-acid-green/10 transition-colors"></div>
                <span
                  className={`font-mono text-[9px] font-bold ${hudEnabled ? "text-dirty-white/70 group-hover:text-acid-green" : "text-acid-green animate-pulse"}`}
                >
                  HUD
                </span>
                <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-dirty-white/20 -rotate-45 translate-x-1/2 -translate-y-1/2"></div>
        ...
        {hudEnabled && (
          <div className="flex flex-col items-end gap-1">
            <div className="w-46 h-4 bg-void-black border border-dirty-white/20 relative">
              <motion.div
                animate={{ width: `${energy}%` }}
                className={`h-full ${energy >= 100 ? "bg-acid-green shadow-[0_0_15px_var(--color-acid-green)]" : "bg-electric-blue"}`}
              />
              <div className="absolute inset-0 flex items-center justify-between px-2 pointer-events-none">
                <span className="text-[8px] text-dirty-white/50 font-mono">
                  ENERGY_LEVEL
                </span>
              </div>
            </div>
            ...
            <AnimatePresence>
              {energy >= 100 && (
                ...
                >
                  ...
                  <span>
                    ULTIMATE_READY:{" "}
                    <kbd className="bg-acid-green text-void-black px-1 rounded text-xs">
                      CTRL
                    </kbd>{" "}
                    +{" "}
                    <kbd className="bg-acid-green text-void-black px-1 rounded text-xs">
      ...
      {projectiles.map((p) => (
        <div
          key={p.id}
          className={`absolute ${p.isUlt ? "bg-hot-pink shadow-[0_0_15px_var(--color-hot-pink)] w-2 h-32" : "bg-acid-green shadow-[0_0_10px_var(--color-acid-green)] w-1 h-6"} rounded-full`}
          style={{
            left: p.x,
            top: p.y,
        ...
        return (
          ...
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
              className="w-full h-full drop-shadow-[0_0_8px_var(--color-hot-pink)]"
            >
              <path
      ...
      {hudEnabled && (
        <AnimatePresence>
          {comboPopups.map((pop) => (
            <motion.div
              key={pop.id}
              initial={{ y: pop.y, x: pop.x, opacity: 1, scale: 0.5 }}
              animate={{ y: pop.y - 100, opacity: 0, scale: 1.5 }}
              className="absolute font-glitch text-acid-green text-xl whitespace-nowrap"
      ...
      <AnimatePresence>
        {explosions.map((exp) => (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="absolute w-12 h-12 border-4 rounded-full"
            style={{
              left: exp.x,
              top: exp.y,
      ...
      >
        <motion.div animate={{ rotate: shipRotation }} className="relative">
          ...
          <svg
            ...
            className="drop-shadow-[0_0_10px_var(--color-acid-green)]"
          >
            <path
            ...
            />
            <motion.path
              animate={{ opacity: [0.6, 1, 0.6], scaleY: [1, 1.8, 1] }}
